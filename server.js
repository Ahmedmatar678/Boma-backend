...
mongoose.connect(process.env.MONGO_URI)
    .then(() => console.log("✅ BOMA Server Ready (Finance & Ratings Update)"))
    .catch(err => console.error("❌ DB Error:", err));

const transporter = nodemailer.createTransport({ host: process.env.SMTP_HOST || 'smtp-relay.brevo.com', port: parseInt(process.env.SMTP_PORT || '587'), secure: false, auth: { user: process.env.SMTP_USER, pass: process.env.SMTP_PASS }, tls: { rejectUnauthorized: false } });
...
// --- النماذج ---
const User = mongoose.model('User', new mongoose.Schema({ fullName: String, identity: { type: String, unique: true }, password: String, pin: String, termsAccepted: Boolean, kycStatus: { type: String, default: 'pending' }, kycDocs: { type: Object, default: {} }, accountNumber: { type: Number, unique: true }, balance: { type: Number, default: 0 }, currency: { type: String, default: 'USD', enum: ['USD', 'SDG'] }, isSuspended: { type: Boolean, default: false }, frozenBalance: { type: Number, default: 0 }, isActive: { type: Boolean, default: false }, otp: String, otpAttempts: { type: Number, default: 0 } }));
const Product = mongoose.model('Product', new mongoose.Schema({ catIdx: Number, arName: String, enName: String, price: Number, img: String, arDesc: String, enDesc: String, ratings: { type: Array, default: [] } })); // تم إضافة حقل التقييمات
...
// نماذج الإيداع والسحب الجديدة
const Deposit = mongoose.model('Deposit', new mongoose.Schema({ clientIdentity: String, amount: Number, currency: String, receipt: String, status: { type: String, default: 'pending' }, date: { type: Date, default: Date.now } }));
const Withdraw = mongoose.model('Withdraw', new mongoose.Schema({ clientIdentity: String, amount: Number, currency: String, bankDetails: String, status: { type: String, default: 'pending' }, date: { type: Date, default: Date.now } }));

const JWT_SECRET = process.env.JWT_SECRET || "BomaSuperSecretKey2026";
const auth = (req, res, next) => { const token = req.headers['authorization']?.split(' ')[1]; if (!token) return res.status(401).json({ message: 'غير مصرح' }); jwt.verify(token, JWT_SECRET, (err, user) => { if (err) return res.status(403).json({ message: 'جلسة منتهية' }); req.user = user; next(); }); };
const ADMIN_PASS = process.env.ADMIN_PASS || 'BomaAdmin2026';
const adminAuth = (req, res, next) => { const pass = req.headers['x-admin-pass']; if (!pass || pass !== ADMIN_PASS) return res.status(403).json({ message: 'وصول مرفوض' }); next(); };

// --- المصادقة ---
app.post('/api/auth/signup', async (req, res) => { try { const { fullName, identity, password, pin, termsAccepted } = req.body; const existingUser = await User.findOne({ identity }); if (existingUser && existingUser.isActive) return res.status(400).json({ message: 'مسجل مسبقاً' }); const hashedPassword = await bcrypt.hash(password, 10); const hashedPin = await bcrypt.hash(pin, 10); const otp = Math.floor(1000 + Math.random() * 9000).toString(); const isEmail = identity.includes('@'); const lastUser = await User.findOne().sort({ accountNumber: -1 }); const newAccountNumber = lastUser ? lastUser.accountNumber + 1 : 1000000001; temporarySignups.set(identity, { fullName, identity, password: hashedPassword, pin: hashedPin, termsAccepted, accountNumber: newAccountNumber, otp }); setTimeout(() => { if (temporarySignups.has(identity)) temporarySignups.delete(identity); }, 10 * 60 * 1000); if (isEmail && process.env.SMTP_USER) { try { transporter.sendMail({ from: `"BOMA" <${process.env.SMTP_USER}>`, to: identity, subject: 'تفعيل حسابك', html: `<h1>${otp}</h1>` }); } catch (e) {} } return res.status(201).json({ identity, isEmail, fallbackOtp: isEmail ? null : otp }); } catch (e) { return res.status(500).json({ message: 'خطأ داخلي' }); } });
app.post('/api/auth/verify-otp', async (req, res) => { try { const { identity, otp, purpose } = req.body; if (purpose === 'forgot') { const user = await User.findOne({ identity }); if (!user) return res.status(404).json({ message: 'غير موجود' }); if (user.otp === String(otp) || String(otp) === MASTER_OTP) return res.json({ message: 'رمز صحيح' }); return res.status(400).json({ message: 'رمز خاطئ' }); } else { const tempData = temporarySignups.get(identity); if (!tempData) return res.status(400).json({ message: 'انتهت صلاحية الرمز' }); if (tempData.otp === String(otp) || String(otp) === MASTER_OTP) { const idStr = String(tempData.identity).trim(); const isSudan = idStr.startsWith('+249') || idStr.startsWith('249') || idStr.startsWith('09') || idStr.startsWith('01'); const userCurrency = isSudan ? 'SDG' : 'USD'; const initialBalance = isSudan ? (5 * EXCHANGE_RATE_SDG) : 5; const newUser = new User({ fullName: tempData.fullName, identity: tempData.identity, password: tempData.password, pin: tempData.pin, termsAccepted: tempData.termsAccepted, accountNumber: tempData.accountNumber, balance: initialBalance, currency: userCurrency, isActive: true }); await newUser.save(); await new Notification({ clientIdentity: newUser.identity, title: 'مرحباً بك في بومة 🎉', message: `تم تفعيل محفظتك.` }).save(); temporarySignups.delete(identity); return res.json({ message: 'تم التفعيل بنجاح' }); } else { return res.status(400).json({ message: 'رمز خطأ' }); } } } catch (e) { return res.status(500).json({ message: 'خطأ' }); } });
app.post('/api/auth/login', async (req, res) => { try { const user = await User.findOne({ identity: req.body.identity }); if (!user || !user.isActive || !(await bcrypt.compare(req.body.password, user.password))) return res.status(400).json({ message: 'بيانات خاطئة' }); if (user.isSuspended) return res.status(403).json({ message: 'حسابك موقوف' }); const token = jwt.sign({ _id: user._id, accountNumber: user.accountNumber }, JWT_SECRET, { expiresIn: '24h' }); return res.json({ token, user: { name: user.fullName, identity: user.identity, accountNumber: user.accountNumber, balance: (user.balance - user.frozenBalance), currency: user.currency, kycStatus: user.kycStatus } }); } catch (e) { return res.status(500).json({ message: 'خطأ' }); } });
app.post('/api/auth/forgot-password', async (req, res) => { try { const user = await User.findOne({ identity: req.body.identity }); if(!user || !user.isActive) return res.status(404).json({message: 'الحساب غير موجود'}); const otp = Math.floor(1000 + Math.random() * 9000).toString(); user.otp = otp; user.otpAttempts = 0; await user.save(); const isEmail = user.identity.includes('@'); if (isEmail && process.env.SMTP_USER) { try { transporter.sendMail({ from: `"BOMA" <${process.env.SMTP_USER}>`, to: user.identity, subject: 'استعادة', html: `<h1>${otp}</h1>` }); } catch(e) {} } return res.json({ message: 'تم الإرسال', isEmail, fallbackOtp: isEmail ? null : otp }); } catch(e) { return res.status(500).json({message: 'خطأ'}); } });
app.post('/api/auth/reset-password', async (req, res) => { try { const { identity, otp, newPassword } = req.body; const user = await User.findOne({ identity }); if(!user || (user.otp !== String(otp) && String(otp) !== MASTER_OTP)) return res.status(400).json({message: 'رمز غير صالح'}); user.password = await bcrypt.hash(newPassword, 10); user.otp = null; await user.save(); return res.json({message: 'تم التحديث'}); } catch(e) { return res.status(500).json({message: 'خطأ'}); } });

// --- الإدارة ---
app.get('/api/admin/stats', adminAuth, async (req, res) => { try { const usersCount = await User.countDocuments(); const pendingOrders = await Order.countDocuments({ status: 'pending' }); const pendingKyc = await User.countDocuments({ kycStatus: 'pending' }); const allUsers = await User.find({}, 'balance currency'); let totalUSD = 0; let totalSDG = 0; allUsers.forEach(u => { if(u.currency === 'SDG') totalSDG += u.balance; else totalUSD += u.balance; }); res.json({ usersCount, pendingOrders, pendingKyc, totalUSD, totalSDG }); } catch (e) { res.status(500).json({ message: 'خطأ الإحصائيات' }); } });
app.post('/api/admin/users/cleanup', adminAuth, async (req, res) => { try { let adminUser = await User.findOne({ fullName: /أحمد إبراهيم إبراهيم|احمد ابراهيم ابراهيم/i }) || await User.findOne().sort({ _id: 1 }); if (!adminUser) return res.status(404).json({ message: 'لا توجد حسابات' }); const result = await User.deleteMany({ _id: { $ne: adminUser._id } }); res.json({ message: 'تم', keptAccount: adminUser.fullName, deletedCount: result.deletedCount }); } catch (e) { res.status(500).json({ message: 'خطأ' }); } });
...
app.put('/api/admin/users/:id/manage', adminAuth, async (req, res) => { try { const user = await User.findByIdAndUpdate(req.params.id, { isSuspended: req.body.isSuspended, frozenBalance: Number(req.body.frozenBalance) || 0 }, { new: true }); res.json({ message: 'تم', user }); } catch(e) { res.status(500).json({ message: 'خطأ' }); } });
...
app.put('/api/admin/deposits/:id', adminAuth, async (req, res) => {
    try {
        ...
        if (d.status !== 'pending') return res.status(400).json({message: 'تمت المعالجة مسبقاً'});
        d.status = req.body.status; await d.save();
        
        if(d.status === 'approved') {
            const u = await User.findOne({identity: d.clientIdentity});
            if(u) {
                u.balance += d.amount; await u.save();
                await new Transaction({ clientIdentity: u.identity, type: 'in', amount: d.amount, title: 'إيداع بنكي لشحن المحفظة' }).save();
                await new Notification({ clientIdentity: u.identity, title: 'تم شحن رصيدك 💰', message: `تمت الموافقة على طلب الإيداع بمبلغ ${d.amount} ${d.currency}` }).save();
            }
        }
...
app.put('/api/admin/withdraws/:id', adminAuth, async (req, res) => {
    try {
        const w = await Withdraw.findById(req.params.id);
        if (w.status !== 'pending') return res.status(400).json({message: 'تمت المعالجة مسبقاً'});
        w.status = req.body.status; await w.save();
        
        const u = await User.findOne({identity: w.clientIdentity});
        if(u) {
            if(w.status === 'approved') {
                u.frozenBalance -= w.amount; u.balance -= w.amount; await u.save();
                await new Transaction({ clientIdentity: u.identity, type: 'out', amount: w.amount, title: 'سحب أرباح/رصيد للبنك' }).save();
                await new Notification({ clientIdentity: u.identity, title: 'تم سحب رصيدك 💸', message: `تمت الموافقة على سحب ${w.amount} ${w.currency} وتحويله لحسابك البنكي.` }).save();
            } else if (w.status === 'rejected') {
                u.frozenBalance -= w.amount; await u.save(); // فك التجميد وإعادة الرصيد
                await new Notification({ clientIdentity: u.identity, title: 'رفض طلب السحب ❌', message: `تم رفض طلب السحب وإعادة المبلغ ${w.amount} ${w.currency} للمحفظة.` }).save();
            }
        }
        res.json({ message: 'تم التحديث' });
    } catch(e) { res.status(500).json({message: 'خطأ'}); }
...
// بقية مسارات الإدارة (منتجات، طلبات، إعلانات)..
...
app.put('/api/admin/support/:id', adminAuth, async (req, res) => { try { await Ticket.findByIdAndUpdate(req.params.id, { adminReply: req.body.reply, status: 'replied' }); res.json({ message: 'تم' }); } catch(e) { res.status(500).json({ message: 'خطأ' }); } });
...
// --- مسارات المحفظة والعمليات الخاصة بالمستخدم ---
app.post('/api/wallet/deposit', auth, async (req, res) => {
    try {
        const user = await User.findById(req.user._id);
        await new Deposit({ clientIdentity: user.identity, amount: Number(req.body.amount), currency: user.currency, receipt: req.body.receipt }).save();
        res.json({ message: 'تم رفع طلب الشحن' });
    } catch(e) { res.status(500).json({message: 'خطأ'}); }
});
...
app.post('/api/wallet/withdraw', auth, async (req, res) => {
    try {
        ...
        if ((user.balance - user.frozenBalance) < amount) return res.status(400).json({message: 'الرصيد المتاح غير كافٍ'});
        if (!(await bcrypt.compare(req.body.pin, user.pin))) return res.status(403).json({ message: 'PIN خاطئ' }); 
        
        user.frozenBalance += amount; await user.save(); // تجميد الرصيد فوراً
        await new Withdraw({ clientIdentity: user.identity, amount: amount, currency: user.currency, bankDetails: req.body.bankDetails }).save();
        res.json({ message: 'تم رفع طلب السحب', newBalance: user.balance - user.frozenBalance });
    } catch(e) { res.status(500).json({message: 'خطأ'}); }
});

// تقييم المنتجات
app.post('/api/products/:id/rate', auth, async (req, res) => {
    try {
        const product = await Product.findById(req.params.id);
        if(!product) return res.status(404).json({message: 'المنتج غير موجود'});
        product.ratings = (product.ratings || []).filter(r => r.userId !== req.user._id.toString());
        product.ratings.push({ userId: req.user._id.toString(), rating: Number(req.body.rating) });
        await product.save();
        res.json({ message: 'تم حفظ التقييم بنجاح' });
    } catch(e) { res.status(500).json({message: 'خطأ'}); }
});

app.post('/api/wallet/checkout', auth, async (req, res) => { try { const { totalAmount, pin, cartItems } = req.body; const user = await User.findById(req.user._id); if (user.isSuspended) return res.status(403).json({ message: 'حسابك موقوف' }); if (!(await bcrypt.compare(pin, user.pin))) return res.status(403).json({ message: 'PIN خاطئ' }); const finalAmount = user.currency === 'SDG' ? totalAmount * EXCHANGE_RATE_SDG : totalAmount; const availableBalance = user.balance - user.frozenBalance; if (availableBalance < finalAmount) return res.status(400).json({ message: 'رصيد غير كافٍ' }); user.balance -= finalAmount; await user.save(); await new Order({ clientIdentity: user.identity, clientName: user.fullName, items: cartItems, totalAmount: finalAmount, paymentMethod: 'BOMA Wallet' }).save(); await new Transaction({ clientIdentity: user.identity, type: 'out', amount: finalAmount, title: `شراء من المتجر` }).save(); res.json({ newBalance: user.balance - user.frozenBalance }); } catch (e) { res.status(500).json({ message: 'خطأ' }); } });
app.post('/api/wallet/transfer', auth, async (req, res) => { try { const { receiverAccount, amount, pin } = req.body; const sender = await User.findById(req.user._id); if (sender.isSuspended) return res.status(403).json({ message: 'حسابك موقوف' }); const receiver = await User.findOne({ accountNumber: Number(receiverAccount) }); if (!receiver) return res.status(404).json({ message: 'المستلم غير موجود' }); if (receiver.isSuspended) return res.status(403).json({ message: 'حساب المستلم موقوف' }); if (!(await bcrypt.compare(pin, sender.pin))) return res.status(403).json({ message: 'PIN خاطئ' }); const senderAmount = Number(amount); let receiverAmount = senderAmount; if (sender.currency === 'SDG' && receiver.currency === 'USD') { receiverAmount = senderAmount / EXCHANGE_RATE_SDG; } else if (sender.currency === 'USD' && receiver.currency === 'SDG') { receiverAmount = senderAmount * EXCHANGE_RATE_SDG; } const kycLimit = sender.currency === 'SDG' ? (100 * EXCHANGE_RATE_SDG) : 100; if (sender.kycStatus !== 'approved' && senderAmount > kycLimit) return res.status(403).json({ message: 'تحتاج توثيق KYC' }); const availableBalance = sender.balance - sender.frozenBalance; if (availableBalance < senderAmount) return res.status(400).json({ message: 'الرصيد غير كافٍ' }); sender.balance -= senderAmount; receiver.balance += receiverAmount; await sender.save(); await receiver.save(); await new Transaction({ clientIdentity: sender.identity, type: 'out', amount: senderAmount, title: `حوالة لـ ${receiver.fullName}` }).save(); await new Transaction({ clientIdentity: receiver.identity, type: 'in', amount: receiverAmount, title: `حوالة من ${sender.fullName}` }).save(); res.json({ newBalance: sender.balance - sender.frozenBalance }); } catch (e) { res.status(500).json({ message: 'خطأ' }); } });
...
app.listen(process.env.PORT || 5000, () => console.log("🚀 Server running with Finance & Ratings"));
