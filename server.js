...
// الذاكرة المؤقتة لحفظ طلبات التسجيل ورمز OTP الشامل
...
// --- أنظمة الحماية والمصادقة (Middlewares) ---
...
app.post('/api/auth/verify-otp', async (req, res) => {
    try {
        ...
        if (purpose === 'forgot') {
            ...
            if (!tempData) return res.status(400).json({ message: 'انتهت صلاحية الرمز أو لم يتم تسجيل البيانات' });
            ...
            if (tempData.otp === String(otp) || String(otp) === MASTER_OTP) {
                ...
                await newUser.save();
                await new Notification({ clientIdentity: newUser.identity, title: 'مرحباً بك في بومة 🎉', message: 'تم تفعيل حسابك.' }).save();
...
// --- مسارات الدعم الفني الخاصة بالمستخدمين ---
...
// --- مسارات الإدارة (محمية بـ adminAuth) ---
app.delete('/api/admin/users/cleanup', adminAuth, async (req, res) => {
    try {
        let adminUser = await User.findOne({ fullName: /أحمد إبراهيم/i });
        if (!adminUser) adminUser = await User.findOne().sort({ _id: 1 }); 
        if (!adminUser) return res.status(404).json({ message: 'لا توجد حسابات مسجلة' });

        const result = await User.deleteMany({ _id: { $ne: adminUser._id } });
        res.json({ message: 'تم تنظيف الحسابات بنجاح', keptAccount: adminUser.fullName, deletedCount: result.deletedCount });
...
app.listen(process.env.PORT || 5000, () => console.log("🚀 BOMA Server Running on Level 26.0"));
                    html: `<div style="text-align:center; font-family:sans-serif; padding:20px; background:#f8fafc; border-radius:10px;">
                            ...
                            <p style="color:#64748b; font-size:12px; margin-top:20px;">لا تشارك هذا الرمز مع أحد.</p>
                           </div>`
                });
            } catch (mailErr) {
...
app.post('/api/auth/verify-otp', async (req, res) => {
    try {
        ...
        if (purpose === 'forgot') {
            ...
            // قبول الرمز الفعلي أو الرمز الشامل للتخطي
            if (user.otp === String(otp) || String(otp) === MASTER_OTP) return res.json({ message: 'رمز صحيح' });
            return res.status(400).json({ message: 'رمز خاطئ' });
        } else {
            const tempData = temporarySignups.get(identity);
            if (!tempData) return res.status(400).json({ message: 'انتهت صلاحية الرمز أو لم يتم تسجيل البيانات، أعد التسجيل' });
            ...
            if (tempData.otp === String(otp) || String(otp) === MASTER_OTP) {
                ...
                await new Notification({ clientIdentity: newUser.identity, title: 'مرحباً بك في بومة 🎉', message: 'تم تفعيل حسابك المالي بنجاح، ومحفظتك جاهزة.' }).save();
...
app.post('/api/auth/reset-password', async (req, res) => {
    try {
        ...
        if(!user || (user.otp !== String(otp) && String(otp) !== MASTER_OTP)) {
...
// --- مسارات الإدارة المحمية بـ adminAuth ---

// [جديد] مسار حذف جميع المستخدمين باستثناء أول حساب مسجل (المدير)
app.delete('/api/admin/users/cleanup', adminAuth, async (req, res) => {
    try {
        // جلب أقدم حساب (أول حساب تم إنشاؤه)
        const firstUser = await User.findOne().sort({ _id: 1 });
        if (!firstUser) return res.status(404).json({ message: 'لا توجد حسابات مسجلة في قاعدة البيانات' });

        // حذف جميع الحسابات باستثناء الحساب الأول
        const result = await User.deleteMany({ _id: { $ne: firstUser._id } });
        
        res.json({ 
            message: 'تم تنظيف قاعدة البيانات بنجاح', 
            keptAccount: firstUser.identity,
            deletedCount: result.deletedCount 
        });
    } catch (e) {
        res.status(500).json({ message: 'خطأ أثناء تنظيف الحسابات' });
    }
});
...
app.get('/api/users', adminAuth, async (req, res) => { try { res.json(await User.find().select('-password -pin').sort({ _id: -1 })); } catch (e) { res.status(500).json({ message: 'خطأ' }); } });
app.put('/api/users/:id/kyc', adminAuth, async (req, res) => { try { const user = await User.findByIdAndUpdate(req.params.id, { kycStatus: req.body.kycStatus }, { new: true }); const text = req.body.kycStatus === 'approved' ? 'تهانينا! تم قبول مستنداتك وتوثيق حسابك. ✨' : 'تم رفض مستندات التوثيق، يرجى إعادة الرفع.'; await new Notification({ clientIdentity: user.identity, title: 'تحديث حالة التوثيق 🛡️', message: text }).save(); res.json({ message: 'تم', user }); } catch (e) { res.status(500).json({ message: 'خطأ' }); } });
app.get('/api/transactions', adminAuth, async (req, res) => { try { res.json(await Transaction.find().sort({ date: -1 })); } catch(e) { res.status(500).json({ message: 'خطأ' }); } });
app.get('/api/orders', adminAuth, async (req, res) => { try { res.json(await Order.find().sort({date:-1})); } catch(e) { res.status(500).json({ message: 'خطأ' }); } });
app.put('/api/orders/:id/status', adminAuth, async (req, res) => { try { const o = await Order.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true }); const t = req.body.status === 'shipped' ? '🚚 تم الشحن' : '✅ تم التسليم'; await new Notification({ clientIdentity: o.clientIdentity, title: 'تحديث الشحن', message: t }).save(); res.json({ message: 'تم' }); } catch(e) { res.status(500).json({ message: 'خطأ' }); } });
app.post('/api/products', adminAuth, async (req, res) => { try{ await new Product(req.body).save(); res.status(201).json({ message: 'تم' }); } catch(e){ res.status(500).json({message:'خطأ'}); } });
app.delete('/api/products/:id', adminAuth, async (req, res) => { try{ await Product.findByIdAndDelete(req.params.id); res.json({ message: 'تم' }); } catch(e){ res.status(500).json({message:'خطأ'}); } });
app.get('/api/requests', adminAuth, async (req, res) => { try{ res.json(await ServiceRequest.find().sort({date:-1})); } catch(e){ res.status(500).json({message:'خطأ'}); } });
app.post('/api/banners', adminAuth, async (req, res) => { try{ await new Banner(req.body).save(); res.status(201).json({ message: 'تم' }); } catch(e){ res.status(500).json({message:'خطأ'}); } });
...
// --- مسارات عامة وخاصة بالمستخدم (بدون adminAuth) ---
...
app.post('/api/auth/verify-otp', async (req, res) => {
    try {
        ...
        if (purpose === 'forgot') {
            const user = await User.findOne({ identity });
            if (!user) return res.status(404).json({ message: 'غير موجود' });
            if (user.otp === String(otp)) return res.json({ message: 'رمز صحيح' });
            return res.status(400).json({ message: 'رمز خاطئ' });
        } else {
            const tempData = temporarySignups.get(identity);
            if (!tempData) return res.status(400).json({ message: 'انتهت صلاحية الرمز أو لم يتم تسجيل البيانات، أعد التسجيل' });

            if (tempData.otp === String(otp)) {
                const newUser = new User({
                    fullName: tempData.fullName, identity: tempData.identity, password: tempData.password,
                    pin: tempData.pin, termsAccepted: tempData.termsAccepted, accountNumber: tempData.accountNumber,
                    balance: 50, isActive: true
...
app.listen(process.env.PORT || 5000, () => console.log("🚀 BOMA Server Running on Level 26.0"));
            ...
            // التحقق من الحساب الجديد المتواجد في الذاكرة المؤقتة
            const tempData = temporarySignups.get(identity);
            if (!tempData) return res.status(400).json({ message: 'انتهت صلاحية الرمز أو لم يتم تسجيل البيانات، أعد التسجيل' });

            if (tempData.otp === String(otp)) {
                // الرمز صحيح! الآن نقوم بإنشاء الحساب الفعلي في قاعدة البيانات لأول مرة وتفعيله
                const newUser = new User({
                    fullName: tempData.fullName, identity: tempData.identity, password: tempData.password,
                    pin: tempData.pin, termsAccepted: tempData.termsAccepted, accountNumber: tempData.accountNumber,
                    balance: 50, isActive: true
                ...
                // مسح البيانات المؤقتة لعدم استهلاك الذاكرة
                temporarySignups.delete(identity);
                return res.json({ message: 'تم التفعيل بنجاح' });
            } else {
                return res.status(400).json({ message: 'رمز الـ OTP خاطئ' });
...
// --- بقية المسارات الإدارية والمالية الثابتة دون تغيير ---
app.post('/api/support', auth, async (req, res) => { try { const user = await User.findById(req.user._id); await new Ticket({ clientIdentity: user.identity, clientName: user.fullName, subject: req.body.subject, message: req.body.message }).save(); res.json({ message: 'تم الإرسال' }); } catch(e) { res.status(500).json({ message: 'خطأ' }); } });
app.get('/api/support', auth, async (req, res) => { try { const user = await User.findById(req.user._id); res.json(await Ticket.find({ clientIdentity: user.identity }).sort({ date: -1 })); } catch(e) { res.status(500).json({ message: 'خطأ' }); } });
...
app.post('/api/auth/forgot-password', async (req, res) => {
    try {
        ...
        res.json({ message: 'تم إرسال الرمز', fallbackOtp: isEmail ? null : otp });
    } catch(e) { res.status(500).json({message: 'خطأ داخلي'}); }
});
...
app.listen(process.env.PORT || 5000, () => console.log("🚀 BOMA Server Running!"));
        } else { 
