...
// 🧠 الذاكرة المؤقتة لحفظ طلبات التسجيل قبل تفعيلها بالـ OTP
const temporarySignups = new Map();

// 🔑 رمز OTP موحد لتسهيل الاختبار وتخطي أي مشاكل في استلام الرمز
...
// حماية مسارات لوحة التحكم (الإدارة)
const ADMIN_PASS = process.env.ADMIN_PASS || 'BomaAdmin2026';
const adminAuth = (req, res, next) => {
    const pass = req.headers['x-admin-pass'];
    if (!pass || pass !== ADMIN_PASS) {
        return res.status(403).json({ message: 'وصول مرفوض: غير مصرح لك بدخول لوحة الإدارة' });
    }
    next();
};

// --- المصادقة والـ OTP المطور دون حفظ مسبق ---
app.post('/api/auth/signup', async (req, res) => {
    try {
        ...
        if (existingUser && existingUser.isActive) {
            return res.status(400).json({ message: 'هذا الحساب مسجل ومفعل مسبقاً' });
        }
        ...
        temporarySignups.set(identity, {
            fullName, identity, password: hashedPassword, pin: hashedPin, termsAccepted, accountNumber: newAccountNumber, otp
        });

        // طباعة الرمز في الكونسول لسهولة التتبع أثناء التطوير
        console.log(`[BOMA System] 🔑 رمز OTP لتفعيل الحساب (${identity}) هو: ${otp}`);

        // مسح البيانات مؤقتاً بعد 10 دقائق لتنظيف الذاكرة
        setTimeout(() => {
            if (temporarySignups.has(identity)) {
