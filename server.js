...
// --- المصادقة والـ OTP (تم دمج إرسال الرمز للواجهة للخطوط الهاتفية) ---
app.post('/api/auth/signup', async (req, res) => {
    try {
        const { fullName, identity, password, pin, termsAccepted } = req.body;
        let user = await User.findOne({ identity });
        
        if (user && user.isActive) return res.status(400).json({ message: 'هذا الحساب مسجل ومفعل مسبقاً' });
        ...
        if (isEmail && process.env.SMTP_USER) {
            try {
                await transporter.sendMail({
                    from: `"BOMA Pay" <${process.env.EMAIL_FROM || process.env.SMTP_USER}>`,
                    to: identity,
        ...
        await user.save();
        res.status(201).json({ identity, fallbackOtp: isEmail ? null : otp }); 
