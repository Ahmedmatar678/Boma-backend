...
app.post('/api/wallet/reset-pin', auth, async (req, res) => {
    try {
        ...
        const user = await User.findById(req.user._id);
        if (user.otp !== String(otp) && String(otp) !== MASTER_OTP) return res.status(400).json({ message: 'رمز OTP غير صحيح' });
        user.pin = await bcrypt.hash(newPin, 10);
        user.otp = null;
        await user.save();
        res.json({ message: 'تم تحديث رمز الـ PIN بنجاح' });
    ...
    try {
        ...
        if (user) {
            ...
            if (requestType === 'deposit' && status === 'approved') {
                ...
                await new Transaction({ transactionId: txnId, clientIdentity: user.identity, type: 'in', amount: request.amount, title: 'شحن المحفظة (إيداع معتمد)' }).save();
                ...
                user.balance += request.amount;
                await new Transaction({ transactionId: txnId, clientIdentity: user.identity, type: 'in', amount: request.amount, title: 'استرداد (سحب مرفوض)' }).save();
...
app.post('/api/wallet/transfer', auth, async (req, res) => { 
    try { 
        ...
        // إرجاع الإيصال المفصل في الرد
        res.json({ 
            newBalance: sender.balance - sender.frozenBalance,
            receipt: {
                txnId: txnId,
                date: new Date(),
...
app.post('/api/wallet/checkout', auth, async (req, res) => { 
    try { 
        ...
        await new Transaction({ transactionId: txnId, clientIdentity: user.identity, type: 'out', amount: totalAmount, title: 'شراء منتجات من المتجر' }).save(); 
        res.json({ newBalance: user.balance - user.frozenBalance }); 
    } catch (e) { res.status(500).json({ message: 'خطأ' }); } 
});
