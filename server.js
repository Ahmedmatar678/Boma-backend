const express = require('express'); // 1. تم تصحيح Const إلى const
...
// 2. حماية مسارات لوحة التحكم (الإدارة)
...
app.post('/api/auth/signup', async (req, res) => {
    try {
        ...
        // 3. مسح البيانات مؤقتاً بعد 10 دقائق لتنظيف الذاكرة (Memory Leak Fix)
        setTimeout(() => {
            if (temporarySignups.has(identity)) {
                temporarySignups.delete(identity);
                console.log(`[System] تم مسح البيانات المؤقتة غير المفعلة للحساب: ${identity}`);
            }
        }, 10 * 60 * 1000);
